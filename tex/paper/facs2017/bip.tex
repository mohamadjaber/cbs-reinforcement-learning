...
\begin{figure}[t]
  \begin{center}
    \mbox{
      \subfigure[Philosopher $P$ and fork $F$ atomic components.]{\label{fig:philo-fork}\scalebox{0.4}{\input{figs/philAndFork.pdf_t}}} \quad
...
\begin{definition}[Priority]
  ...
  abbreviate $(a,a')\in \prio$ by $a \prec a'$. Adding the priority model $\prio$ over $\Gamma(\{B_1, \ldots, B_n\})$ defines a new composite component $B = \prio\big(\Gamma(\{B_1, \ldots, B_n\})\big)$ noted $\mathtt{prio}(C)$ and whose behavior is defined by $(Q, \Gamma, \goesto_\prio)$, where $\goesto_\prio$ is the least set of transitions satisfying the following rule:
...
\end{definition}
%
An interaction $a$ is enabled in $\mathtt{prio}(C)$ whenever $a$ is enabled in $C$ and $a$ is maximal according to $\mathtt{prio}$ among the active interactions in $C$.
