...
Next, the engine executes the interaction and stores both the dynamical transition and its usefulness (i.e., reward) in a replay memory data set, $\mathcal{D}$. We use a technique known as experience replay~\cite{memory-replay} where we store the transitions executed at each step, $\left( \bm{q_{t}}, a_{t}, r_{t+1},  \bm{q_{t+1}}\right)$ in a replay memory $\mathcal{D}$. By using memory replay the behavior distribution is averaged over many of its previous transitions, smoothing out learning and avoiding oscillations or divergence in the parameters. 
