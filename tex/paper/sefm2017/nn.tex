...
\foreach \l [count=\i] in {1,|\Gamma|}
  \draw [->] (output-\i) -- ++(1,0)
    node [above, midway] {\quad $\mathbb{P}(q, a_{\l})$};
...
\end{tikzpicture}
\caption{A high-level depiction of an artificial NN showing three layers as well as the weight connections between them.}
\end{figure}
